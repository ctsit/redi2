...
END
    ...
    cp /vagrant/apache_files/apache-ssl.conf /etc/apache2/sites-available/default-ssl.conf
    ...
    cp /vagrant/apache_files/apache-default.conf /etc/apache2/sites-available/000-default.conf
...
function populate_db () {
    ...
    log "Evaluating schema in ${SCHEMA_FOLDER}"
    log "Showing EPOCH at: ${DB_EPOCH_VERSION}"
    for dir in `find $SCHEMA_FOLDER -maxdepth 1 -type d | sort --version-sort | grep -A1000 ${DB_EPOCH_VERSION} | tail -n +2`;
    do
        log "Checking for upgrade.sql in ${dir}"
...
function reset_db {
    . /vagrant/.env
    . /vagrant/redcap_deployment_functions.sh
